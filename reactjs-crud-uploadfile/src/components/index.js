      ...
      listData = this.props.items.map((item, index) => {
        return (
          <tr key={index}>
            ...
            <th>{item.name}</th>
            <th>{(item.img) ? // check co anh hay k
              item.img.map((img, key) => {
                return (
                  <img key={key} alt="img" src={img} height="90px" />
                )
              }) : null
            }</th>
            <th>
              ...
              <button onClick={() => this.props.unlinkImg({ id: item._id, textSearch: this.state.text, activePage: this.state.activePage })}>
                unlinkImg
              </button>
            </th>
          </tr>
